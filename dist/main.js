...
async function welcome() {
    let rainbowTitle = chalkAnimation.rainbow(`Welcome to the Ramzan's "Library Management System`);
    await sleep();
    rainbowTitle.stop();
...
function addBook(id, title, author, isAvailable) {
    if (books.some((book) => book.id.toString() === id.toString())) {
        console.log(chalk.red(`Book with Id ${id} is already exist. Please enter a another id.`));
    }
...
function editBook(id, title, author, isAvailable) {
    let book = books.find((book) => book.id.toString() === id.toString());
    if (book) {
        (book.title = title),
            (book.author = author),
            (book.isAvailable = isAvailable),
            console.log(chalk.green(`Book having the Id number ${id} is edited successfully.`));
    }
    else {
...
function deleteBook(id) {
    ...
    console.log(chalk.green(`Book having the Id number ${id} is deleted successfully.`));
}
...
async function displayMenu() {
    ...
    switch (userInputs.action) {
        case "Add a new book":
            ...
            addBook(addAnswer.id, addAnswer.title, addAnswer.author, addAnswer.isAvailable);
            break;
        case "Edit an existing book":
            ...
            editBook(editAnswer.id, editAnswer.title, editAnswer.author, editAnswer.isAvailable);
            break;
